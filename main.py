...
class InstrScr(Screen):
    def __init__(self, **kwargs):
        ...
        lbl1 = Label(text='Enter name:', halign='right')
    ...
    def next(self):
        ...
        if age == False or age < 7:
            ...
            popup = Popup(title='Error', content=Label(text='Age needs to be 7 and up'), size_hint=(None, None), size=(600, 600), pos_hint={'center_x': 0.5, 'center_y': 0.5})
            popup.open()
        else:
            self.in_name.text = ''
            self.in_age.text = '7'
...
class PulseScr(Screen):
    def __init__(self, **kwargs):
        ...
        lbl1 = ScrollLabel('Count Heart rate', textcolor = '#FFFFFF')
...
class CheckSits(Screen):
    def __init__(self, **kwargs):
        ...
        lbl1 = ScrollLabel('Do 30 sit-ups', textcolor = '#FFFFFF')
...
class PulseScr2(Screen):
    def __init__(self, **kwargs):
        ...
        self.lbl1 = ScrollLabel('Count Heart rate', textcolor = '#FFFFFF')
        self.lbl_sec = Seconds(15, textcolor = '#FFFFFF')
        self.lbl_sec.bind(done=self.sec_finished)
        ...
        line2 = BoxLayout(size_hint=(0.8, None), height='30sp')
        lbl_result2 = Label(text='Result after a break:', halign='right')
    ...
    def sec_finished(self, instance, value):
        if value:
            if self.stage == 0:
                self.stage = 1
                self.lbl1.set_text('Take a break')
                self.lbl_sec.restart(30)
                self.in_result1.set_disabled(False)
            elif self.stage == 1:
                self.stage = 2
                self.lbl1.set_text('Count Heart rate')
...
class Result(Screen):
    def __init__(self, **kwargs):
        ...
        self.btn = Button(text='Finish', size_hint=(0.3, 0.2), pos_hint={'center_x': 0.5})
        self.btn.background_color = btn_color
        self.btn.on_press = self.next
